...
class UploadModel extends Model
{
     ...
     * @param array $data reference to data that will be passed to db
     *
    ...
    protected function _storeUploadedFile($path, array $file_data, array & $data, $action)
    {
        if($action == 'update')
        {
            ...
            $this->_getPath($data['model'], $data['id']);
        }
        ...
        if (!$this->_afterStoring($path, $file_data, $data, $action))
            return false;
     ...
     * Delete path
     * @author m.augustynowicz
     * @author m.jutkiewicz (param $data)
     *
     * @param string $path file location (generated by _getPath())
     * @param array $data data of deleted file (from database)
     *
     * @return boolean false on any error
     */
    protected function _deletePath($path, array $data)
    {
        if (!$this->_beforeDeleting($path, $data))
            return false;

        g('Functions')->rmrf($path); // will echo "deleting $path"

        if (!$this->_afterDeleting($path, $data))
            return false;
        return true;
    }
     ...
     */
    protected function _beforeDeleting($path, array $data)
    {
        return true;
    }
     ...
     * @return boolean returning false will stop everything
     */
    protected function _afterDeleting($path, array $data)
    {
        return true;
    }
     ...
     * @return boolean returning false will stop everything
     */
    protected function _beforeStoring($path, array $file_data, array & $data, $action)
    {
        return true;
    }
     ...
     * @return boolean returning false will stop everything
     */
    protected function _afterStoring($path, array $file_data, array & $data, $action)
    {
        return true;
    }
